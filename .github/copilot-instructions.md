## Lessons Learned: Subprocess Environment Propagation
- When running scripts via subprocesses (e.g., with Python's subprocess.run), environment variables loaded in the parent process (including those from .env files) are NOT automatically available to child processes unless explicitly passed.
- Always load .env in the parent and use env=os.environ.copy() when invoking subprocesses to ensure all secrets and credentials are available.
- This prevents authentication and configuration errors that only appear in multi-script or orchestrated workflows.
...
- Always load .env in the parent and use env=os.environ.copy() when invoking subprocesses to ensure all secrets and credentials are available.
- This prevents authentication and configuration errors that only appear in multi-script or orchestrated workflows.
# Copilot Instructions for ColorLange

## Overview
ColorLange is a novel HSV pixel-based programming language that represents an experimental approach to visual programming. Programs are encoded as HSV color grids in standard image formats, executed by a custom virtual machine. This project demonstrates innovative concepts in spatial programming, compression-native language design, and visual debugging techniques.

## Important Note
ColorLange builds on established visual programming concepts (like Piet from 2001) with unique contributions in HSV-based encoding, pixel-level instructions, and compression optimization. This project serves as both innovative research into visual programming paradigms and a foundation for evolution toward addressing genuine market opportunities in energy-efficient computing, constraint-driven development, and hardware optimization democratization.

## Key Components

### 1. Virtual Machine (`colorlang/virtual_machine.py`)
- Executes ColorLange programs parsed from images.
- Implements a rich instruction set (e.g., `ADD`, `PRINT`, `HALT`).
- Updates shared memory regions for communication with the host.
...
### 4. Examples (`examples/`)
- Contains prebuilt ColorLange programs (e.g., `hello_world.png`, `fibonacci_sequence.png`).
...
- **API-First Scraping Principle**: Before building any scraper or automation against a public API, always review and document the API’s rate limits, quotas, authentication requirements, and best practices. Understand the API’s intended usage patterns and limitations to avoid inefficient or blocked scraping workflows.
...
- **Performance Validation**: Measure real performance vs theoretical projections; distinguish simulation from reality.
