"""
scrape_openbenchmarking.py

Scrapes benchmark results from OpenBenchmarking.org and inserts them into the energy_lang knowledge base.
...
Usage:
    python scrape_openbenchmarking.py
"""
import os
import requests
import psycopg2
...
def scrape_openbenchmarking():
    url = "https://openbenchmarking.org/result"
    resp = requests.get(url)
    soup = BeautifulSoup(resp.text, "html.parser")
    # TODO: Parse benchmark result links and extract data
    # For each result, call insert_benchmark(...)
    pass

def main():
    scrape_openbenchmarking()
